...
export default function MockInterview() {
  ...
  const speak = useCallback(
    (text: string, onFinished?: () => void) => {
      ...
      utterance.onstart = () => {
        if (!isCompletedRef.current) setStatus("speaking");
      };
  ...
  async function startInterview() {
    ...
    try {
      const openingStyles = [
        "briefly introduce yourself and ask the student to introduce themselves and share a bit about their background.",
        "briefly introduce yourself and ask the student what motivated them to pursue this field.",
        "briefly introduce yourself and ask the student to walk you through their resume or recent projects.",
        "briefly introduce yourself and ask the student what they're most proud of so far in their studies or work.",
        "briefly introduce yourself and ask the student to describe themselves in a few sentences, as they would in a real interview.",
      ];
      const chosenOpening =
        openingStyles[Math.floor(Math.random() * openingStyles.length)];

      const firstQuestion = await askAI(
        `You are a friendly, realistic human interviewer conducting a ${currentMode.label} for a student preparing for campus placements. ${roleContext}\n\nFor your FIRST question, ${chosenOpening} Do NOT ask any hard technical questions yet. Keep it warm, natural, and welcoming. Ask ONLY this first introductory question. Do not provide evaluation.`,
      );
            ...
            <div className="flex min-h-0 flex-1 flex-col items-center px-3 py-2.5 sm:px-6 sm:py-4">
              <div className="flex min-h-0 w-full max-w-3xl flex-1 flex-col gap-2.5 sm:gap-3">
                ...
                <div className="flex shrink-0 items-center justify-between gap-2 rounded-2xl border border-gray-200 bg-white px-3 py-2.5 sm:justify-center sm:gap-3 sm:px-4 dark:border-gray-800 dark:bg-gray-900">
                  <div
                    className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-violet-600 to-purple-500 text-base shadow-sm ${status === "speaking" ? "animate-pulse" : ""
                      }`}
                  >
                    🤖
                    {status === "speaking" && (
                  ...
                  <div className="ml-1 flex shrink-0 items-center gap-1 sm:ml-2">
                    {Array.from({ length: QUESTION_LIMIT }).map((_, i) => (
                      <span
                        key={i}
                        className={`h-1.5 rounded-full transition-all ${i < questionNumber
