...
export default function Dashboard() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      // Real 7-day trend built from actual session_scores rows, grouped by day
      ...
      const last7Days = Array.from({ length: 7 }, (_, i) => {
        ...
        const dayInterviewScores = scores
          .filter(
            (s) =>
              s.feature?.toLowerCase().includes("interview") &&
              new Date(s.created_at).toDateString() === dayKey,
          )
          .map((s) => s.overall_score)
          .filter((score): score is number => typeof score === "number");

        const dayCommScores = scores
          .filter(
            (s) =>
              s.feature?.toLowerCase().includes("communication") &&
              new Date(s.created_at).toDateString() === dayKey,
          )
          .map((s) => s.overall_score)
          .filter((score): score is number => typeof score === "number");
        ...
        return {
          date: days[d.getDay()],
          interview: dayInterviewScores.length
            ? Number(
                (
                  dayInterviewScores.reduce((a, b) => a + b, 0) /
                  dayInterviewScores.length
                ).toFixed(1),
              )
            : null,
          communication: dayCommScores.length
            ? Number(
                (
                  dayCommScores.reduce((a, b) => a + b, 0) /
                  dayCommScores.length
                ).toFixed(1),
              )
            : null,
        };
